...
get_all_entries_in_directory :: proc(in_path : string) -> []DiskEntry {
    path := in_path;
    buf : [1024]byte;
    if(path[len(path)-1] != '*') {
...
get_all_entries_strings_in_directory :: proc(in_dir_path : string, full_path : bool = false) -> []string {
    path_buf : [win32.MAX_PATH]u8;
    dir_path := in_dir_path;
